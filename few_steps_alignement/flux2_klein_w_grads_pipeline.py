...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def __call_with_grad__(
        ...
        """
        Same as __call__ but returns image tensor with gradients attached.

        Returns:
            torch.Tensor: Generated image tensor with gradients, shape (B, C, H, W), range [0, 1]
        """
        ...
        # Decode through VAE (with gradients)
        # Returns image in [-1, 1] range, (B, C, H, W) format
        image = self.vae.decode(latents, return_dict=False)[0]
